...
export default function VentasPage() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                // Seller API (Sales) - orders for products I sell
                ...
                if (response.ok) {
                    ...
                    const formattedOrders = (data.orders || []).map((order: any) => ({
                        ...
                        total: order.total_price > 0
                            ...
                            : "Vía Cotización",
                        image: order.product_image || "/placeholder.svg",
                        slug: order.product_slug || order.product_id,
                        is_read: order.is_read_seller ?? true
                    }))
    ...
    const prepOrders = pedidos.filter(p => ["Preparación", "Pendiente", "pending"].includes(p.estado)).length
        ...
        <div className="space-y-6 p-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">Pedidos recibidos de tus productos</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card>
                    ...
                    <CardContent>
                        <p className="text-2xl font-bold text-yellow-600">{prepOrders}</p>
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-3">
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="relative">
                            ...
                            <Input
                                placeholder="Buscar por comprador, producto..."
                                value={searchTerm}
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Pedidos realizados por compradores a tus productos</CardDescription>
                </CardHeader>
                <CardContent>
                    {filteredPedidos.length === 0 ? (
                        <div className="text-center py-12">
                            <p className="text-muted-foreground">No tienes ventas registradas</p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-4">
                            {filteredPedidos.map((ped) => (
                                <Link
                                    key={ped.fullId}
                                    href={`/dashboard/ventas/${ped.fullId}`}
                                            ...
                                            <div className="flex flex-col md:flex-row justify-between items-start gap-2 mb-2">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-muted-foreground line-clamp-1">
                                                        <span className="font-semibold text-foreground">Comprador:</span> {ped.cliente}
                                                    </p>
                                                </div>
